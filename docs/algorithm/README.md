...
10번 문서(Bone 가중치/스키닝)는 충돌 감지 알고리즘이 아니라 `모델임포터_Mesh저장` 사이클에서 파생된 학습 노트이지만, 충돌 검사 파이프라인과 직접 연결되는 내용이라 이 폴더에 함께 둔다.
...
11. [10_Bone_가중치_스키닝.md](10_Bone_가중치_스키닝.md) — 스켈레탈 애니메이션의 정점 가중치 구조, 향후 이 메쉬들을 충돌 검사에 연결할 때 왜 Mesh Support() 대신 뼈별 Capsule 프록시를 써야 하는지.
